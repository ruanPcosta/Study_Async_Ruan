...
from . models import Apostilas, ViewApostila
from django.contrib.messages import constants
from django.contrib import messages
# Create your views here.
def adicionar_apostilas(request):
    if request.method == "GET":
        apostilas = Apostilas.objects.filter(user = request.user)
        ...
        apostila.save()
        messages.add_message(request, constants.SUCCESS, 'A apostila foi registrada com exito')
        
        return redirect('/apostilas/adicionar_apostilas')

def apostila(request, id ):
    apostila = Apostilas.objects.get(id = id )
